...
export const isNullOrWhiteSpace = (str) =>
  !str || !str.toString().replace(/(<([^>]+)>)|^\s+|\s+$|\s+/g, "");
...
export const renderBreadcrumbs = (menuContent, pageContext) => {
  ...
  let parent = menuContent.find(
    (mc) => mc.strapi_id === current?.strapi_parent?.id,
  );
  ...
  function addItems(parent, menuContent, breadcrumbItems) {
    if (parent) {
      ...
      parent = menuContent.find(
        (mc) => mc.strapi_id === parent?.strapi_parent?.id,
      );
      return addItems(parent, menuContent, breadcrumbItems);
    }
