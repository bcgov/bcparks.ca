...
export const query = graphql`
  {
    ...
    allStrapiSearchArea(sort: { rank: ASC }) {
      nodes {
        searchAreaName
        strapi_id
      }
...
export default function FindAPark({ location, data }) {
  ...
  const sortedActivityItems = orderBy(
    ...
    [(activity) => activity.activityName.toLowerCase()],
    ["asc"],
  );
  const areaItems = data.allStrapiSearchArea.nodes.map((area) => {
    const filterCount =
      areasCount?.find((areaCount) => areaCount.key === area.strapi_id)
        ?.doc_count || 0;
    return {
  ...
  const campingTypeItems = data.allStrapiCampingType.nodes.map((camping) => {
    const filterCount =
      campingsCount?.find(
  ...
  const [qsAreas, setQsAreas, qsAreasInitialized] = useQueryParamString(
    "sa",
    "",
  );
  const [qsParkCampingTypes, setQsParkCampingTypes, qsCampingsInitialized] =
    useQueryParamString("c", "");
  const [qsActivities, setQsActivities, qsActivitiesInitialized] =
    useQueryParamString("a", "");
  const [qsFacilities, setQsFacilities, qsFacilitiesInitialized] =
    useQueryParamString("f", "");
  const [qsLocation, setQsLocation, qsLocationInitialized] =
    useQueryParamString("l", "");
  const [searchText, setSearchText, searchTextInitialized] =
    useQueryParamString(
      "q",
      location.state?.searchText ? location.state.searchText : "",
    );
  ...
  const [selectedCity, setSelectedCity] = useState(
    location.state?.selectedCity ? location.state.selectedCity : [],
  );
  ...
  const [currentPage, setCurrentPage, currentPageInitialized] =
    useQueryParamString("p", 1);
  ...
  const handleSearch = (clickedCity) => {
    ...
    if (clickedCity?.length > 0) {
      ...
      const enteredCity = searchCities.filter(
        (city) => city.cityName.toLowerCase() === cityText.toLowerCase(),
      );
  ...
  const params = useMemo(() => {
    ...
    if (selectedParkCampingTypes.length > 0) {
      params.campings = selectedParkCampingTypes.map(
  ...
  const queryParamStateSyncComplete = () => {
    ...
    return (
      ...
      (hasCity || !qsLocation) &&
      Math.sign(qsParkCampingTypes.length) ===
        Math.sign(selectedParkCampingTypes.length) &&
        ...
        .then(([resultResponse1, resultResponse2]) => {
          if (resultResponse1.status === 200) {
            const total = parseInt(
              resultResponse1.data.meta.pagination.total,
              10,
            );
            ...
            setActivitiesCount(
              resultResponse1.data.meta.aggregations.activities.buckets,
            );
            setFacilitiesCount(
              resultResponse1.data.meta.aggregations.facilities.buckets,
            );
            setCampingsCount(
              resultResponse1.data.meta.aggregations.campings.buckets,
            );
          } else {
            setSearchResults([]);
            setTotalResults(0);
          ...
          if (request2 && resultResponse2.status === 200) {
            const total = parseInt(
              resultResponse2.data.meta.pagination.total,
              10,
            );
  ...
  useEffect(() => {
    ...
    if (selectedActivities.length === 0 && qsActivities.length > 0) {
      setSelectedActivities(
        activityItems.filter((x) => arr(qsActivities).includes(x.value)),
      );
    }
    if (
      selectedParkCampingTypes.length === 0 &&
      qsParkCampingTypes.length > 0
    ) {
      setSelectedParkCampingTypes(
        campingTypeItems.filter((x) =>
          arr(qsParkCampingTypes).includes(x.value),
        ),
      );
    }
    if (selectedFacilities.length === 0 && qsFacilities.length > 0) {
      setSelectedFacilities(
        facilityItems.filter((x) => arr(qsFacilities).includes(x.value)),
      );
    }
    ...
    if (qsLocation && qsLocation !== "0") {
      const selectedCities = searchCities.filter(
        (city) => city.strapi_id.toString() === qsLocation,
      );
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [qsActivities, qsParkCampingTypes, qsFacilities, qsAreas, qsLocation]);
  ...
  useEffect(() => {
    ...
    const shouldRedirect =
      ...
      searchResults[0].protectedAreaName.toLowerCase() ===
  ...
  const finalResults = useMemo(() => {
    return selectedCity.length > 0 &&
      selectedCity[0].latitude !== 0 &&
      selectedCity[0].longitude !== 0 &&
      hasParksWithinFifty(searchResults)
      ? totalResultsWithinFifty
      : totalResults;
          ...
          <div className="row g-0">
            ...
            <div className="search-results-quick-filter col-12 col-lg-3 d-none d-lg-block pe-3">
              ...
              <div className="park-links">
                <p className="subtitle mb-2">
                  <b>More ways to find a park</b>
                </p>
                <div>
                  <GatsbyLink to="/find-a-park/a-z-list">
                    A–Z park list
                  </GatsbyLink>
                  <br />
                      ...
                      {selectedCity.length > 0 &&
                        ...
                        selectedCity[0].longitude !== 0 && (
                          <>
                            ...
                            within{" "}
                            <b>
                              {hasParksWithinFifty(searchResults) ? 50 : 100}{" "}
                              km{" "}
                            </b>
                      ...
                      <>
                        ...
                        selectedCity[0].longitude !== 0 ? (
                          <>
                            ...
                            {hasParksWithinFifty(searchResults) &&
                              hasParksWithinOneHundred(searchResults) && (
...
export const Head = () => (
  <Seo
    title="Find a park"
    description="Search for the webpages of parks and protected areas across British Columbia. Get detailed information on camping and other activities at specific BC Parks."
  />
);
