...
const MegaMenu = ({ content, menuMode }) => {
  ...
  if (typeof window !== "undefined") {
    menuCollection = document.getElementsByClassName(
      "menu-children-exist--true menu-level-0-children",
    );
    ...
    (arr, level) => {
      ...
      arr.forEach((item) => {
        ...
        if (item.treeLevel < 3) {
          ...
          // process children
          item.strapi_children = sortedTree(
            item.strapi_children || [],
            level + 1,
  ...
  let sections = content.filter(
    (item) => item.strapi_parent?.url === ROOT_MENU_URL,
  );
                ...
                {isExternalUrl(item.url) ? (
                  ...
                  >
                    ...
                    <FontAwesomeIcon
                      ...
                      className="menu-button__title--icon"
                    />
                  </a>
                ) : (
                  <Link
