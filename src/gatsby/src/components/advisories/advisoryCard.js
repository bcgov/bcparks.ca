...
const AdvisoryCard = ({ advisory, parkInfoHash }) => {
  ...
  const hasAdvisoryDateDisplay =
    advisory.isAdvisoryDateDisplayed && advisoryDate;
  const hasUpdatedDateDisplay = advisory.isUpdatedDateDisplayed && updatedDate;
  const hasEffectiveDateDisplay =
    advisory.isEffectiveDateDisplayed && effectiveDate;
  ...
  const showNaturalResourceDistricts =
    advisory.naturalResourceDistricts?.length > 0;

  const checkAdditionalParks = () => {
    const advisoryFireCentres = advisory.fireCentres.map((c) => {
      return c.id;
    });
    const advisoryFireZones = advisory.fireZones.map((z) => {
      return z.id;
    });
    const advisoryRegions = advisory.regions.map((r) => {
      return r.id;
    });
    const advisorySections = advisory.sections.map((s) => {
      return s.id;
    });
    const advisoryNaturalResourceDistricts =
    ...
    const parkCameFromNaturalResourceDistrict = (paKey) => {
      const parkNaturalResourceDistricts =
        parkInfoHash[paKey]?.naturalResourceDistricts || [];
      return parkNaturalResourceDistricts.some((x) =>
        advisoryNaturalResourceDistricts.includes(x),
      );
    };
    ...
    for (const pa of advisory.protectedAreas) {
      ...
      if (
        showNaturalResourceDistricts &&
        !parkCameFromNaturalResourceDistrict(paKey)
      ) {
        return true;
      }
    }
  ...
  const combinedArray = [
    ...(showFireCentres
      ? advisory.fireCentres.map((item) => ({
          ...item,
          type: "fireCentre",
          name: item.fireCentreName,
        }))
      : []),
    ...(showFireZones
      ? advisory.fireZones.map((item) => ({
          ...item,
          type: "fireZone",
          name: item.fireZoneName,
        }))
      : []),
    ...(showRegions
      ? advisory.regions.map((item) => ({
          ...item,
          type: "region",
    ...
    ...(showSections
      ...
      : []),
    ...(showNaturalResourceDistricts
      ? advisory.naturalResourceDistricts.map((item) => ({
          ...item,
          type: "naturalResourceDistrict",
          name: item.naturalResourceDistrictName,
        }))
      : []),
  ];
  ...
  return (
    <Row key={advisory.id} tabIndex="-1" className="advisory-card g-0">
        ...
        <div className="card-content">
          ...
          {advisory.protectedAreas.length > 5 ? (
            ...
            <div className="d-flex flex-wrap">
              ...
              {advisory.sites.length > 0 &&
                advisory.sites.map((site, index) =>
                  advisory.protectedAreas.map(
                    (park, index) =>
          ...
          <div className="card-content--bottom">
            ...
            <div className="type">
              ...
              <small>
                <b>{advisory.eventType.eventType}</b>
              </small>
            </div>
          </div>
        </div>
