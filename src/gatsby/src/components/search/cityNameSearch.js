...
const CityNameSearch = ({
  ...
  const cityOptions = (optionLimit) => {
    ...
    const filteredCities = cities.filter(
      (city) =>
        city.cityName.toLowerCase().startsWith(cityTextLower) ||
        city.cityName.toLowerCase().includes(` ${cityTextLower}`),
    );
    const sortedCities = filteredCities.slice().sort((a, b) => {
      if (
        a.cityName.toLowerCase().startsWith(cityTextLower) &&
        !b.cityName.toLowerCase().startsWith(cityTextLower)
      ) {
        return -1;
      } else if (
        !a.cityName.toLowerCase().startsWith(cityTextLower) &&
    ...
    return [
      ...(cityText ? sortedCities.slice(0, optionLimit) : []),
      ...[currentLocation],
    ];
  };
  ...
  useEffect(() => {
    if (
      ...
      selectedItems[0].strapi_id !== 0 &&
      cityText !== selectedItems[0].cityName
    ) {
  ...
  return (
    <>
      ...
      <Typeahead
        ...
        placeholder=" "
        className={`has-text--${
          selectedItems.length > 0 || cityText.length > 0 ? "true" : "false"
        } is-dropdown-open--${
          ...
          return (
            <Form.Group controlId="city-search-typeahead">
              ...
              {hasNoMatch && (
                ...
                >
                  No match. Please check your spelling or try a larger city in
                  B.C.
                </span>
              )}
            </Form.Group>
            ...
            {results.map((city, index) => {
              return city.strapi_id !== 0 ? (
                <MenuItem option={city} position={index} key={index}>
                  <HighlightText city={city.cityName} input={cityText} />
                </MenuItem>
              ) : (
                <MenuItem
                  option={city}
