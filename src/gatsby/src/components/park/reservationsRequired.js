...
export default function ReservationsRequired({ operations }) {
  ...
  const hasFrontcountryGroupReservations =
    operations.hasFrontcountryGroupReservations;
  const hasFrontcountryCabinReservations =
  ...
  const hasBackcountryGroupReservations =
    operations.hasBackcountryGroupReservations;
  const hasBackcountryShelterReservations =
    operations.hasBackcountryShelterReservations;
  const hasBackcountryWildernessReservations =
      ...
      <>
        ...
        <ul>
          ...
          {(hasFrontcountryGroupReservations ||
            hasBackcountryGroupReservations) && (
            <li>
              <Link to="/reservations/group-camping">Group camping</Link>
            </li>
          )}
          {hasGroupPicnicReservations && (
            <li>
              <Link to="/reservations/picnic-shelters">Picnic shelters</Link>
            </li>
          )}
          {(hasBackcountryReservations ||
            hasBackcountryShelterReservations ||
          ...
          {hasDayUsePass && (
            <li>
              <Link to="/reservations/day-use-passes">Day-use pass</Link>
            </li>
          )}
          {reservationLinks?.length > 0 &&
            reservationLinks.map((link, i) => (
              <li
                key={i}
                dangerouslySetInnerHTML={{
                  __html: link.content.data.content.replace(/<\/?p>/g, ""),
                }}
              ></li>
            ))}
        </ul>
