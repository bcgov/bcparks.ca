    ...
    if ("isIgnored" in parkFeature) {
      if (parkFeature.isIgnored == null) {
        ...
        if (parkFeature.parkFeatureTypeId) {
          const parkFeatureType = parkFeatureTypes.find(
            (type) => type.featureTypeId === parkFeature.parkFeatureTypeId,
          );
          if (parkFeatureType) {
...
export default function ParkAccessStatus({
    ...
    return (
      advisories?.some((advisory) => {
        const accessStatusId =
          advisory.accessStatus?.id || advisory.accessStatusId;
        if (!accessStatusId) return false;

        const accessStatus = accessStatusList.find(
          (status) => status.strapi_id === accessStatusId,
        );
  ...
  const advisoriesWithSeasonal = useMemo(() => {
    ...
    if (
      newAdvisories.some((advisory) => advisory.id === SEASONAL_ADVISORY_ID) ||
      hidesSeasonalAdvisory
    ) {
    ...
    <div className="access-status-icon">
      {accessStatus && (
        <>
          ...
          {!hideComma && advisoriesWithSeasonal.length > 0 && ", "}
