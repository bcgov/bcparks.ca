...
export default function VisitResponsibly({
  ...
  const hasFrontcountryCamping = campings.some((c) =>
    frontcountryCampingCodes.includes(c.campingType?.campingTypeCode),
  );
  const hasBackcountryCamping = campings.some((c) =>
    backcountryCampingCodes.includes(c.campingType?.campingTypeCode),
  );
  const hasMarineCamping = campings.some((c) =>
    marineCampingCode.includes(c.campingType?.campingTypeCode),
  );
  const hasWinterCamping = campings.some((c) =>
    winterCampingCode.includes(c.campingType?.campingTypeCode),
  );
  const hasWinterActivities = activities.some((a) =>
    winterActivityCode.includes(a.activityType?.activityCode),
  );
  const hasFishingOrHunting = activities.some((a) =>
    activityCodes.includes(a.activityType?.activityCode),
  );
  ...
  return (
    <>
      ...
      <p>
        Follow these guides to ensure your activities are safe, respectful, and
