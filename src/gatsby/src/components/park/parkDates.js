...
export const ReservationButtons = ({ campingTypeCode, parkOperation }) => {
      ...
      if (code === "group-camping" && parkOperation) {
        ...
        if (hasFrontcountry) {
          buttons = buttons.filter(
            (button) => button.fieldName !== "backcountryGroupReservationUrl",
          );
        }
      }
      return buttons;
  ...
  return (
    ...
    getReservationButtons(campingTypeCode).map(
      (button, index) =>
        parkOperation &&
        parkOperation[button.fieldName] && (
          <a
            key={index}
    ...
    >
      {itemCount > 1 ? (
        ...
        <div id={parkFeatureId} className="accordion-toggle">
          <HtmlContent className="accordion-header">{displayName}</HtmlContent>
        </div>
      )}
...
export default function ParkDates({
  data,
  parkOperation,
  ...
  const expandLabel = (campingType) => {
    return allExpanded
      ? `Collapse all ${campingType.toLowerCase()}`
      : `Expand all ${campingType.toLowerCase()}`;
  };
        ...
        parkFeatures.length > 0 && (
          <>
            <Row className="align-items-center my-4">
              <Col>
                <h4 className="mb-0">{data.campingType.pluralName}</h4>
              </Col>
              <Col className="reservation-button-container" xs="12" sm="auto">
                <ReservationButtons
                  campingTypeCode={data.campingType.campingTypeCode}
