...
export const CampingType = ({
  ...
  const handleExpandClick = () => {
    ...
    const element = document.getElementById(
      `${camping?.campingType.campingTypeCode}`,
    );
    if (element) {
  ...
  const expandLabel = (campingName) => {
    return expanded
      ? `Show less about ${campingName.toLowerCase()}`
      : `Show more about ${campingName.toLowerCase()}`;
  };
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      if (h3 && hr) {
        ...
        const height =
          hr?.getBoundingClientRect().top - h3.getBoundingClientRect().top;
    ...
    <div id={camping?.campingType.campingTypeCode} className="park-camping">
      ...
      >
        <div className="d-flex align-items-center mb-4">
          <StaticIcon
            name={camping?.campingType?.icon || "information"}
            size={36}
          />
        ...
        <HtmlContent
          ariaHidden={hasExpandCondition && !expanded}
          className="park-camping-description"
        >
          {hasExpandCondition
            ? expanded
              ? campingDescription
        ...
        {!camping.hideStandardCallout &&
          !isNullOrWhiteSpace(
            camping.campingType?.appendStandardCalloutText?.data
