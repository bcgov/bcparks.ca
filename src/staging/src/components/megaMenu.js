...
const MegaMenu = ({ content, menuMode }) => {
  ...
  const toggleMenu = (e) => {
    if (!isMenuOpen) { // menu is closed, will open now
      menuReset() // select the root
    }
    setIsMenuOpen(!isMenuOpen) // toggle open state
  }

  const sortedTree = useCallback((arr, level) => {
    ...
    arr.forEach(item => {
      ...
      item.orderCode = (item.parent?.treeLevel > 0) ? item.parent.orderCode + "." + item.order : item.order;
      ...
      if (item.strapiChildren === undefined) {
        ...
        let itemChildren = menuContent.filter(child => child.strapiParent?.id === item.id)
        item.strapiChildren = itemChildren || []
      }
      if (item.treeLevel < 3) {  // Enforcing max 3 levels
        // process children
  ...
  useEffect(() => { // create sorted + structured menuTree from menuContent
  ...
  useEffect(() => { // make deep copy of content to process, will trigger useEffect above
        ...
        {item.hasChildren && (
          <>
            ...
            <nav className={'menu-level menu-level--' + item.treeLevel}>
              ...
              <ul className="menu-button-list">
                ...
                {item.strapiChildren.map((page, index) => (
                  <li key={index}
                    ...
                    className={'menu-button menu-button--' + (selections[page.treeLevel] === page ? "selected" : "unselected")}>
                      <span className="menu-button__title"
                        role="button"
                        tabIndex={0}
              ...
              {item.strapiChildren.map((page, index) => (
                  <div key={index} className={'menu-children menu-children-exist--' + page.hasChildren + ' menu-level-' + item.treeLevel +'-children menu-children--'  + (page === selections[page.treeLevel] ? "selected" : "unselected")}>
