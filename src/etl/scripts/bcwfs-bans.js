...
const loadData = async function () {
    ...
    try {
        const response = await axios.get(`${process.env.STRAPI_URL}/api/fire-zones?fields=fireZoneName`);
    ...
    try {
        const response = await axios.get(`${process.env.STRAPI_URL}/api/fire-centres?fields=fireCentreName`);
    ...
    try {
        const response = await axios.get(`${process.env.STRAPI_URL}/api/fire-ban-prohibitions?populate[fireCentre][fields][0]=id&populate[fireZone][fields][0]=id`);
    ...
    for (const ban of deletions) {
        try {
            await axios.delete(`${process.env.STRAPI_URL}/api/fire-ban-prohibitions/${ban.id}`, { headers: httpReqHeaders });
        } catch (error) {
            logger.error(error);
            process.exit(1);
        }
    ...
    for (const ban of insertions) {
        try {
            await axios.post(`${process.env.STRAPI_URL}/api/fire-ban-prohibitions/`, { data: ban.attributes }, { headers: httpReqHeaders });
        } catch (error) {
            logger.error(error);
            process.exit(1);
        }
    ...
    if (insertions.length > 0 || deletions.length > 0) {
        ...
        // get a list of existing bans from Strapi
        try {
            await axios.post(`${process.env.STRAPI_URL}/api/fire-ban-prohibitions/propagate`, {}, { headers: httpReqHeaders });
