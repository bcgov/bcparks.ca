...
module.exports = async (ctx, next) => {
  ...
  if (
    ...
    try {
      ...
      if (ctx.request.query && ctx.request.query.token) {
        ...
        decodedToken = await strapi.plugin('users-permissions').services.jwt.getKCToken(ctx);
        if (decodedToken) {
          tokenType = "keycloak";
        } else {
          // get information if token is strapi type
          decodedToken = await strapi.plugin('users-permissions').services.jwt.getToken(ctx);
          if (decodedToken) {
            tokenType = "strapi";
          }
        }
      ...
      if (decodedToken) {
        if (tokenType === "keycloak") {
          ...
          if (decodedToken.resource_access?.['staff-portal']?.roles) {
            ...
            if (roleMatch) {
              ctx.state.user = await strapi.plugin('users-permissions').services.user.fetch({ email: API_USER_EMAIL });
            } else {
              throw new Error(
          ...
          ctx.state.user = await strapi.plugin('users-permissions').services.user.fetchAuthenticatedUser(id);
        }
      }
    } catch (err) {
  ...
  if (permission.policy) {
    return await strapi.plugin('users-permissions').config.policies[
      permission.policy
    ](ctx, next);
  }
