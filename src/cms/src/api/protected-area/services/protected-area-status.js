...
const getPublishedPublicAdvisories = async () => {
  return await strapi.entityService.findMany('api::public-advisory.public-advisory', {
...
const getProtectedAreaStatus = async (ctx) => {
  ...
  const regionsData = await strapi.entityService.findMany('api::region.region', {
    limit: -1,
    populate: '*',
  });
  const sectionsData = await strapi.entityService.findMany('api::section.section', {
    limit: -1,
    populate: '*',
  });
  const fireCentresData = await strapi.entityService.findMany('api::fire-centre.fire-centre', {
    limit: -1,
    populate: '*',
  });
  const activityTypesData = await strapi.entityService.findMany('api::activity-type.activity-type', {
    limit: -1,
    populate: '*',
  });
  const facilityTypesData = await strapi.entityService.findMany('api::facility-type.facility-type', {
    limit: -1,
    populate: '*',
  });
  const linkTypesData = await strapi.entityService.findMany('api::link-type.link-type', {
    limit: -1,
    populate: '*',
  });
  const parkNamesAliases = await strapi.entityService.findMany('api::park-name.park-name', {
    limit: -1,
    populate: '*',
    filters: {
      parkNameType: {
  ...
  const campfireBanData = await strapi.entityService.findMany('api::fire-ban-prohibition.fire-ban-prohibition', {
    filters: {
