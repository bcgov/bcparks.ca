...
async function up(knex) {
    ...
    if (await knex.schema.hasTable('access_statuses')) {
        ...
        await knex('access_statuses').where({ access_status: 'Park is under evacuation order' }).update({ precedence: 2, color: 'red', group_label: 'Closed' });

        await knex('access_statuses').where({ access_status: 'Restricted; permit required' }).update({ precedence: 3, color: 'red', group_label: 'Permit required' });
        ...
        await knex('access_statuses').where({ access_status: 'Warning' }).update({ precedence: 8, color: 'yellow', group_label: 'Visit with caution' });

        await knex('access_statuses').where({ access_status: 'Inaccessible' }).update({ precedence: 100, color: 'red', group_label: 'DUPLICATE' });
        
        await knex('access_statuses').where({ access_status: 'Park is inaccessible' }).update({ precedence: 9, color: 'yellow', group_label: 'Limited access', access_status: 'Inaccessible' });

        await knex('access_statuses').where({ access_status: 'Trail access affected' }).update({ precedence: 11, color: 'yellow', group_label: 'Limited access' });

        // need to reword
        await knex('access_statuses').insert({ access_status: 'Open but no vehicle access', precedence: 98, color: 'blue', group_label: 'Limited access' });
