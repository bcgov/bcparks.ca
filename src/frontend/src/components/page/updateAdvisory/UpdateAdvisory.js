...
export default function UpdateAdvisory({ page: { header, setError } }) {
  ...
  const [advisoryStatusName, setAdvisoryStatusName] = useState();
  ...
  useEffect(() => {
    Promise.all([
      ...
      const publicAdvisory = res[3].data;

      console.log(publicAdvisory);

      setHeadline(publicAdvisory.Title);
      setDescription(publicAdvisory.Description);
      setEventType(publicAdvisory.event_type.id);

      setUrgency(publicAdvisory.urgency.id);
      if (publicAdvisory.advisory_status.id === 4) setIsApproved(true);

      setAdvisoryStatusName(publicAdvisory.advisory_status.AdvisoryStatus);
      setAdvisoryStatus(4);
      if (publicAdvisory.AdvisoryDate)
        setStartDate(publicAdvisory.AdvisoryDate.substring(0, 16));
      if (publicAdvisory.EndDate)
        setEndDate(publicAdvisory.EndDate.substring(0, 16));
      if (publicAdvisory.ExpiryDate)
        setExpiryDate(publicAdvisory.ExpiryDate.substring(0, 16));

      const locations = publicAdvisory.protected_areas.map((p) => p.ORCS);
      setLocations([...locations]);
      console.log(locations);

      const selectedLocations = publicAdvisory.protected_areas.map((p) => ({
        label: p.ProtectedAreaName,
        value: p.ORCS,
      }));
                ...
                <div className="col-lg-8 col-md-8 col-sm-12">
                  <Input
                    input={{
                      ...advisoryStatusInput,
                      styling: "bcgov-editable-white",
                      value: advisoryStatusName,
                    }}
                    onChange={(event) => {
                      setAdvisoryStatusName(event);
                    }}
                  />
                </div>
              </div>
